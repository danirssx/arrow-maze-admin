...
```bash
npm ci
VITE_API_BASE_URL=https://api.arrowmaze.example npm run build   # → dist/
```
...
env var rather than passing it inline.
...
- **Vercel** — `vercel.json` (`buildCommand`, `outputDirectory: dist`, SPA `rewrites`). Import
  the repo; set `VITE_API_BASE_URL` under *Project → Settings → Environment Variables*.
...
```
CORS_ORIGIN=http://localhost:8081,http://localhost:5173,https://admin.arrowmaze.example
```
